...
const defaultClassNames = (variant: Variant) => ({
	label: 'TimePicker flex items-center gap-2',
...
interface TimePickerProps extends BruteComponent<
	HTMLInputElement,
	['label', 'span', 'input', 'button']
> {
	label?: React.ReactNode;
	variant?: Variant;
	defaultValueToCurrentTime?: boolean;
    setToCurrentButton?: boolean;
    setToCurrentButtonText?: string;
	setTime: (prev: string) => void;
	time: string;
...
const TimePicker: React.FC<TimePickerProps> = ({ 
	...
	label = 'Time',
	variant = 'initial',
	defaultValueToCurrentTime = false,
	...
	useEffect(() => {
		if (defaultValueToCurrentTime)
			setTimeToCurrent();
	}, [defaultValueToCurrentTime]);
